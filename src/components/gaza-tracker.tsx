  ...
  return (
    <div className="gaza-tracker-stats space-y-4">
      <div>
        ...
        <p><small className="text-muted-foreground">Source: Palestinian Ministry of Health in Gaza as of {new Date(tracker.update_date).toLocaleDateString()}</small></p>
      </div>
      ...
      {tracker.deaths_chart_url && (
        <div className="relative w-full aspect-video">
           <Image src={tracker.deaths_chart_url} alt="Gaza death tracker chart" layout="fill" objectFit="contain" className="rounded-lg" />
        </div>
      )}
      {tracker.famine_chart_url && (
        <div className="relative w-full aspect-video">
          <Image src={tracker.famine_chart_url} alt="Gaza famine tracker chart" layout="fill" objectFit="contain" className="rounded-lg" />
        </div>
       )}
    </div>
  );
